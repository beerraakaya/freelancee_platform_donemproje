...
export default function FreelancerProfil() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5000/api/freelancers/${userId}` , {credentials: "include", });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data.message || "Profil alınamadı.");
        setP(data);
      } catch (e) {
        alert(e.message);
        navigate("/freelancer_bul", { replace: true });
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="ap-wrap">
      <div className="ap-card">
        <div className="ap-head">
          ...
          <div className="ap-title">Freelancer Profili</div>
        </div>
        ...
        <div className="ap-body">
          <div className="ap-left">
            ...
            <div className="ap-name">{p.ad} {p.soyad}</div>
          </div>
          ...
          <div className="ap-right">
            <div className="ap-row">
              ...
              {githuburl ? (
                <a className="ap-link" href={githuburl} target="_blank" rel="noreferrer">{p.github}</a>
            ...
            <div className="ap-row">
              ...
              {linkedinurl ? (
                <a className="ap-link" href={linkedinurl} target="_blank" rel="noreferrer">{p.linkedin}</a>
              ) : <div className="ap-empty">Yok</div>}
            </div>
